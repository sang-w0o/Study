...
- HTTP(HyperText Transfer Protocol)은 WWW(World Wide Web)상에서의 소통을 위한 표준화된 방식으로  
  1997년에 사용되기 시작한 통신 수단이다. 1997년부터 현재까지 이 프로토콜에는 여러 가지의 수정사항이 있었다.  
...
- 단 하나의 TCP Connection만을 사용해도 된다는 것은 HTTPS Protocol의 성능도 향상시켜준다.  
  ...
  사용하도록 한다. 만약 해당 세션이 종료되고, 새로운 세션이 시작되려면 그에 맞게 key도 새롭게 발급해야 한다.  
...
- 클라이언트와 서버 모두 하나의 TCP Connection에서 처리되지 않은 요청들에 대한 정보를 저장할 수 있는  
  버퍼(Buffer)의 크기가 각각 제한되어 있다. 이 버퍼가 있기에 용량이 큰 요청들에 대해 유연성(Flexibility)을  
  가질 수 있으며, 서버에서 클라이언트의 네트워크 속도와 클라이언트에서 서버로의 네트워크 속도가 다른 상황에 대해  
  잘 처리할 수 있다.

- 하지만 버퍼의 크기가 충분하지 않은 경우도 생긴다. 예를 들어, 네트워크 속도는 느리고, 클라이언트의 버퍼 크기는 적은데  
  서버가 클라이언트에게 매우 큰 용량의 데이터를 보내서 클라이언트가 이에 대한 처리를 할 수 없는 상황이 있다.  
  마찬가지로, 만약 클라이언트가 서버에게 큰 용량의 이미지, 비디오를 보낼 때 서버의 버퍼 크기가 내용물보다 작아  
  overflow가 발생할 수도 있다. 이는 뒤따라오는 추가적인 packet들에 대한 loss가 발생하게 한다.

- 이런 Buffer Overflow가 발생하는 상황을 피하기 위해, 요청을 한 자(Sender)가 요청을 받는 자(Receiver)에게  
...
- 이러한 Flow Control의 자유성은 리소스에 대한 적절한 전략을 수립할 때 큰 도움이 된다.  
  ...
  보여줄 것이다. 이렇게 서버와 클라이언트가 각각의 Flow Control을 정의할 수 있는 것은 웹 애플리케이션에서  
