...
- Flow의 컬렉션은 항상 호출되는 coroutine 내에서 발생한다. 예를 들어 `simple`이라는 flow가 있다고 해보자.  
...
- 위 결과에서 유의해야할 또다른 점은 `flowOn()`이 flow가 순차적으로 수행되는 것을 변경했다는 것이다.  
  Collection은 하나의 coroutine("coroutine #1")에서 진행되고, emission은 또다른 coroutine("coroutine #2")에서 수행된다.  
  그리고 "coroutine #1"과 "coroutine #2"는 병렬적으로 수행된다.  
