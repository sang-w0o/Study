...
<h2>Structured Concurrency with async</h2>
...
- 위 코드 처럼 병렬적으로 수행되는 함수(async를 사용하여)가 CoroutineScope내에서 작동되도록 하면  
  만약 코드가 수행되는 도중 예외가 단져지면 해당 coroutineScope내에서 작동하던 모든 coroutine들의 작업이 중단되는 것을 보장할 수 있다.
...
- 참고 문서: <a href="https://kotlinlang.org/docs/composing-suspending-functions.html">문서</a>
